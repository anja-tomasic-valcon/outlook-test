...
test('E2E: receiver can flag a received email', async ({ browser }, testInfo) => {
  ...
  try {
    ...
    // Optional: small visual assertion that a flag icon exists in the message row (aria-hidden icons are not relied on)
    // const rowFlagIcon = receiver.page.locator('[role="option"], [role="row"]').filter({ hasText: subject }).locator('[aria-hidden="true"] >> img').first();
    // await rowFlagIcon.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {});
